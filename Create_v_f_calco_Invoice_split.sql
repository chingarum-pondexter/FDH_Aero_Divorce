...
*/
Alter VIEW [etl].[v_f_calco_Invoice_split] AS
...
SELECT
    ...
    -- ,1										 AS DivisionId
    -- ,case 
    --     when LEFT(inv.[INVC_NUMBER], 1) = 'C' AND LEFT(inv.[INVC_NUMBER], 2) <> 'CM' AND LEFT(inv.[INVC_NUMBER], 2) <> 'CA' then 9
    --     when LEFT(inv.[INVC_NUMBER], 4) = 'CM-C' then 9 else 
